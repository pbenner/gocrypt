...
func TestAES3(t *testing.T) {
  ...
  target := []byte{0x39, 0x25, 0x84, 0x1d, 0x02, 0xdc, 0x09, 0xfb, 0xdc, 0x11, 0x85, 0x97, 0x19, 0x6a, 0x0b, 0x32}
  out1   := make([]byte, len(input))
  out2   := make([]byte, len(input))

  cipher, _ := NewAESCipher(key)

  cipher.Encrypt(input, out1)
  cipher.Decrypt(out1,  out2)

  if !Bits(out1).Equals(target) {
    t.Error("aes encryption test failed")
  }
  if !Bits(out2).Equals(input) {
    t.Error("aes decryption test failed")
  }
